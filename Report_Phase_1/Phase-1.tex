...
\answerbox{The average internet user has accounts, on websites and apps.. The human brain can only remember a few passwords well. Because of this most people. Use the same password over and over or pick simple ones that are easy to guess. This is a security problem. When a site gets hacked and passwords are leaked hackers often try those logins on other popular sites. This method, called stuffing can break into accounts that the user never meant to expose. Password managers address this problem directly by allowing a user to generate and store strong, unique passwords for every account while only needing to remember a single master password. This project aims to build a functional password manager from first principles, applying core data structures and object-oriented design concepts to solve a problem millions of people face daily.}
